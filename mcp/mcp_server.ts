 ...
 * Using a factory allows for multi-tenant SSE support where each connection 
...
const transportType = process.argv.includes("--transport=sse") ? "sse" : "stdio";

if (transportType === "sse") {
    const app = express();
    app.use(express.json());

    // Map to track active sessions and their transports
    ...
    app.get("/sse", async (req, res) => {
        ...
        // Store transport by its sessionId (available after connect)
    ...
    const port = 3001;
    app.listen(port, () => {
        console.error(`Finance Tools MCP Server started on SSE: http://localhost:${port}/sse`);
    });
} else {
    const server = createFinanceServer();
    const transport = new StdioServerTransport();
    await server.connect(transport);
    console.error("Finance Tools MCP Server started on Stdio");
}